...
An autonomous AI agent that curates daily music recommendations across multiple genres and delivers them via email. Built with Python, Gemini, and the Spotify API.
...
1. **Fetches real candidates** from Spotify's catalog across configurable genres (rock, indie, electronic, house, classical…)
2. **An LLM curates the best picks** — Gemini 3 Flash analyzes the pool, selects the most interesting tracks, and writes a personalized reason for each one
...
│   ├── music_search.py      # Spotify search and candidate generation
...
```
Spotify API                    Gemini 3 Flash                 SQLite
    │                               │                           │
    ├── Search by genre ──►  Pool of ~50-100 real tracks        │
    │                               │                           │
    │                        Select best 5 + write reasons      │
    │                               │                           │
    │                               ├── Check against ◄─────────┤
    │                               │   history (no repeats)    │
    │                               │                           │
...
```python
GENRES = ["rock", "indie", "electronic", "house", "classical"]
SONGS_PER_DAY = 5
```

To add a new genre, just add it to the `GENRES` list and ensure a matching entry exists in the `SEARCH_QUERIES` dictionary in `music_search.py`.
...
- HTML email templating with inline CSS for cross-client compatibility
...
- 🎧 Integration with Last.fm for richer music discovery
